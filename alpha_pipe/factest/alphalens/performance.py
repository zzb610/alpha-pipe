...
def factor_alpha_beta(factor_data,
    ...
    for i in range(len(returns.columns.values)):
        ...
        ret_type = returns.columns.values[i]

        x = universe_ret[ret_type].values
        y = returns[ret_type].values
        x = add_constant(x)
...
def quantile_turnover(quantile_factor, quantile, ret_type, period=1):
    """
...
def factor_rank_autocorrelation(factor_data, ret_type, period=1):
    """
