...
class DatasetProvider(abc.ABC):
    ...
    def dataset_processor(instruments_d, column_names, start_time, end_time, freq):
        ...
        if len(new_data) > 0:
            ...
            data = pd.concat(new_data, names=["asset"], sort=False)
            data = DiskDatasetCache.cache_to_origin_data(data, column_names + ['day'])
        else:
            data = pd.DataFrame(columns=column_names + ['day'])
        return data
...
def register_all_wrappers(C):
    ...
    module = get_module_by_module_path("alpha_pipe.qlib_data.data")
    ...
    register_wrapper(Cal, _calendar_provider, "alpha_pipe.qlib_data.data")
    logger.debug(f"registering Cal {C.calendar_provider}-{C.calendar_cache}")

    register_wrapper(Inst, C.instrument_provider, "alpha_pipe.qlib_data.data")
    ...
    if getattr(C, "feature_provider", None) is not None:
        ...
        register_wrapper(FeatureD, feature_provider, "alpha_pipe.qlib_data.data")
    ...
    if getattr(C, "expression_provider", None) is not None:
        ...
        register_wrapper(ExpressionD, _eprovider, "alpha_pipe.qlib_data.data")
    ...
    register_wrapper(DatasetD, _dprovider, "alpha_pipe.qlib_data.data")
    logger.debug(f"registering DataseteD {C.dataset_provider}-{C.dataset_cache}")

    register_wrapper(D, C.provider, "alpha_pipe.qlib_data.data")
