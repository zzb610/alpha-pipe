...
def register_all_wrappers(C):
    ...
    module = get_module_by_module_path("alpha_pipe.qlib.data")
    ...
    register_wrapper(Cal, _calendar_provider, "alpha_pipe.qlib.data")
    logger.debug(f"registering Cal {C.calendar_provider}-{C.calendar_cache}")

    register_wrapper(Inst, C.instrument_provider, "alpha_pipe.qlib.data")
    ...
    if getattr(C, "feature_provider", None) is not None:
        ...
        register_wrapper(FeatureD, feature_provider, "alpha_pipe.qlib.data")
    ...
    if getattr(C, "expression_provider", None) is not None:
        ...
        register_wrapper(ExpressionD, _eprovider, "alpha_pipe.qlib.data")
    ...
    register_wrapper(DatasetD, _dprovider, "alpha_pipe.qlib.data")
    logger.debug(f"registering DataseteD {C.dataset_provider}-{C.dataset_cache}")

    register_wrapper(D, C.provider, "alpha_pipe.qlib.data")
