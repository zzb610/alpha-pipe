...
class FactorAnalyzer(object):
    ...
    def __init__(self, ret_names, groupby, quantiles, periods, binning_by_group, zero_aware):
    ...
    def set_factor_data(self, factor_data):
        self._clean_factor_data = factor_data
    ...
    def calc_average_cumulative_return_by_quantile(self,ret_name,periods_before, periods_after, demeaned, group_adjust):
        ...
        - False: 不使用行业中性化后的收益
        """
        returns = self._clean_factor_data[['return({})'.format(ret_name)]].unstack()
    ...
    def calc_cumulative_returns(self, ret_name, period, demeaned, group_adjust):
        ...
        """
        ret_name = 'return({})'.format(ret_name)
        factor_returns = self.calc_factor_returns(demeaned=demeaned, group_adjust=group_adjust)[ret_name]
        freq = '{}B'.format(period)
    ...
    def plot_returns_table(self, demeaned, group_adjust):
        ...
        pl.plot_returns_table(
            self.calc_factor_alpha_beta(demeaned=demeaned, group_adjust=group_adjust),
    ...
    def plot_factor_auto_correlation(self, rank):
        ...
        for name in self._ret_names:
            pl.plot_factor_rank_auto_correlation(self.calc_autocorrelation(rank=rank)[name],name)
    ...
    def plot_top_bottom_quantile_turnover(self):
        ...
        for name in self._ret_names:
            pl.plot_top_bottom_quantile_turnover(quantile_turnover_rate[name], name)
    ...
    def plot_cumulative_returns(self,demeaned,group_adjust):
        ...
        for name, period in zip(self._ret_names, self._periods):
            cum_ret = self.calc_cumulative_returns(name, period, demeaned, group_adjust)
    ...
    def plot_top_down_cumulative_returns(self, demeaned, group_adjust):
        ...
        f, ax = plt.subplots(1, 1, figsize=(18, 6))
        for name, period in zip(self._ret_names, self._periods):
            cum_ret = self.calc_top_down_cumulative_returns(name, period, demeaned, group_adjust)
            cum_ret.plot(ax=ax, lw=3, alpha=0.6)
    ...
    def plot_cumulative_returns_by_quantile(self,demeaned, group_adjust):
        ...
        for name, period in zip(self._ret_names, self._periods):
            f, ax = plt.subplots(1, 1, figsize=(18, 6))
            cum_ret = self.calc_cumulative_return_by_quantile(name, period, demeaned, group_adjust)
    ...
    def create_returns_tear_sheet(self, demeaned, group_adjust, by_group):
        ...
        self.plot_quantile_returns_bar(by_group=False,
                                       ...
                                       group_adjust=group_adjust)
        pl.plt.show()
        self.plot_cumulative_returns(demeaned=demeaned, group_adjust=group_adjust)
        pl.plt.show()
        self.plot_cumulative_returns_by_quantile(demeaned=demeaned,group_adjust=group_adjust)
        self.plot_top_down_cumulative_returns(demeaned=demeaned,group_adjust=group_adjust)
        pl.plt.show()
    ...
    def create_information_tear_sheet(self, group_adjust, by_group, ic_method, theoretical_dist):
        ...
        pl.plt.show()
        self.plot_ic_qq(group_adjust=group_adjust, method=ic_method, theoretical_dist=theoretical_dist)
        pl.plt.show()
        if by_group:
            self.plot_ic_by_group(group_adjust=group_adjust, method=ic_method)
        else:
            self.plot_monthly_ic_heatmap(group_adjust=group_adjust, ic_method=ic_method)
        pl.plt.show()
    ...
    def create_full_tear_sheet(self, demeaned, group_adjust, by_group, avgretplot, ic_method,ic_theoretical_dist,turnover_rank,std_bar):
        ...
        pl.plt.show()
        print("\n-------------------------\n")
        self.plot_information_table(group_adjust=group_adjust, method=ic_method)
        self.plot_ic_ts(group_adjust=group_adjust, method=ic_method)
        pl.plt.show()
        self.plot_ic_qq(group_adjust=group_adjust, method=ic_method, theoretical_dist=ic_theoretical_dist)
        pl.plt.show()
        if by_group:
            self.plot_ic_by_group(group_adjust=group_adjust, method=ic_method)
        else:
